...
| `ONLYTTY_CLUSTER_PROJECT` | — | GCP project to discover relay peers in; set → libcluster's GCE strategy forms the BEAM cluster so sessions resolve cluster-wide via `:global`. Unset → single node |
| `ONLYTTY_CLUSTER_VALUE` | `onlytty` | value of the `cluster_name` instance label the GCE strategy filters peers by |
