defmodule OnlyttyWeb.ClientIPTest do
  ...
  setup do
    prev = Application.get_env(:onlytty, :trusted_proxy_hops)
  ...
  test "no proxy (default): keys on the direct peer and ignores X-Forwarded-For" do
    ...
    assert ClientIP.resolve(conn({198, 51, 100, 7}, "127.0.0.1, 10.0.0.1")) == {198, 51, 100, 7}
  end

  test "hops=0 behaves like no proxy" do
    Application.put_env(:onlytty, :trusted_proxy_hops, 0)
  ...
  test "hops=2: a chain of two trusted proxies" do
    Application.put_env(:onlytty, :trusted_proxy_hops, 2)
