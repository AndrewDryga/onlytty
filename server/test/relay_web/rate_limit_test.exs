defmodule RelayWeb.RateLimitTest do
  @moduledoc "Per-IP throttling of POST /api/sessions."
  # async: false — these toggle the global rate-limit config.
  use RelayWeb.ConnCase, async: false

  alias Relay.RateLimit

  setup do
    prev_max = Application.get_env(:relay, :rate_limit_max)
    prev_win = Application.get_env(:relay, :rate_limit_window_ms)

    on_exit(fn ->
      Application.put_env(:relay, :rate_limit_max, prev_max)
      Application.put_env(:relay, :rate_limit_window_ms, prev_win)
    end)
  ...
  test "allows up to the limit, then throttles with a positive retry-after" do
    Application.put_env(:relay, :rate_limit_max, 2)
    Application.put_env(:relay, :rate_limit_window_ms, 60_000)
  ...
  test "resets after the window elapses" do
    Application.put_env(:relay, :rate_limit_max, 1)
    Application.put_env(:relay, :rate_limit_window_ms, 50)
