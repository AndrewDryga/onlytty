defmodule RelayWeb.RelaySocket do
  @moduledoc """
  ...
  def handle_in({data, opcode: :binary}, state) do
    ...
    if is_pid(state.peer), do: send(state.peer, {:relay_binary, data})
    {:ok, state}
  end
  ...
  def handle_info({:relay_binary, data}, state) do
    {:push, [{:binary, data}], state}
  end

  # Control-plane JSON from the Session (peer_join / peer_left).
  def handle_info({:relay_control, json}, state) do
