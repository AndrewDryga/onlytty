defmodule RelayWeb.SocketController do
  @moduledoc """
  ...
  viewer socket sends `{"t":"busy"}` then closes (handled in `RelaySocket`).
  """

  use RelayWeb, :controller

  alias Relay.{Session, SessionStore}
  ...
  defp upgrade(conn, state) do
    conn
    |> WebSockAdapter.upgrade(RelayWeb.RelaySocket, state,
